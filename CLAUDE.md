...
### Adopted in 2.5.0 — return parcels as outgoing (do not refactor away)
...
- **Returns come from the parcels list, not the sent-shipments endpoint.**
  ...
  *receiver* of the original parcel, not the sender of record — so it
  ...
  excluded set somewhere to go instead of being silently dropped).
- **New `DhlCoordinator` attributes**: `returning` (active return parcels,
  sorted by `planned_from`) and `delivered_outgoing` (completed returns,
  sorted by `delivered_at` descending, same delivered-filter options as
  `delivered`). Deliberately on `DhlCoordinator`, not
  `DhlSentShipmentsCoordinator` — see above.
- **New sensors** `DhlReturningParcelsSensor`
  (`{user_id}_returning_parcels`) and `DhlDeliveredOutgoingParcelsSensor`
  (`{user_id}_delivered_outgoing_parcels`), both summary-only (list
  attribute, no per-parcel entities — same pattern as
  `DhlSentShipmentsSensor`). Both unique_ids **must** stay in
  `non_parcel_unique_ids` in `sensor.py`.
