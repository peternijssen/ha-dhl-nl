...
### `coordinator.py`
- `DhlCoordinator` — polls `async_get_parcels()`, applies `filter_active_parcels()` / `filter_delivered_parcels()` (both exclude returns) for the incoming lists, and `filter_active_returns()` / `filter_delivered_returns()` (both require `isReturn`) for the outgoing-return lists — all four filter the *same* raw parcels payload
- `DhlSentShipmentsCoordinator` — polls `async_get_sent_shipments()`, applies `filter_active_sent_shipments()` (keeps only `type == "outgoing"` and active categories). This is the account holder's own DHL-registered shipments; it never contains webshop-generated return labels, since the account holder isn't their sender of record
...
- `DhlReturningParcelsSensor` — active return parcels on their way back to the shipper (`coordinator.returning`)
- `DhlDeliveredOutgoingParcelsSensor` — return parcels that have arrived back at the shipper (`coordinator.delivered_outgoing`)
- `DhlSentShipmentsSensor` — single summary sensor for the account holder's own outgoing shipments (separate `DhlSentShipmentsCoordinator`); no per-shipment entities are created
...
| `DhlReturningParcelsSensor` | `{userId}_returning_parcels` | `abc123_returning_parcels` |
| `DhlDeliveredOutgoingParcelsSensor` | `{userId}_delivered_outgoing_parcels` | `abc123_delivered_outgoing_parcels` |
