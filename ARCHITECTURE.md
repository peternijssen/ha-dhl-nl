...
Both outgoing sensors are `CoordinatorEntity[DhlCoordinator]` (not `DhlSentShipmentsCoordinator`) purely as an implementation choice — the two coordinators share the same refresh interval, so update-notification timing is not meaningfully different either way.
