...
async def async_setup_entry(
    ...
    - Summary sensors for active and delivered return parcels (own-shipment
      returns generated by a webshop, filtered from the same parcels list)
    - A summary sensor for outgoing active shipments (the account holder's
      own sent-shipments, from the separate sent-shipments endpoint)
    """
    ...
    non_parcel_unique_ids = {
        ...
        f"{user_id}_returning_parcels",
        f"{user_id}_delivered_outgoing_parcels",
    ...
    entities.append(DhlReturningParcelsSensor(coordinator=coordinator, user_info=user_info))
    entities.append(DhlDeliveredOutgoingParcelsSensor(coordinator=coordinator, user_info=user_info))
...
class DhlReturningParcelsSensor(CoordinatorEntity[DhlCoordinator], SensorEntity):
    """Sensor reporting active return parcels on their way back to the shipper.

    Sourced from the main parcels coordinator (filtered on ``isReturn``), not
    the separate sent-shipments coordinator — a webshop-generated return
    label never appears there since the account holder isn't its sender of
    record. See ``filter_active_returns``.
    """

    _attr_has_entity_name = True
    _attr_translation_key = "returning_parcels"
    _attr_state_class = SensorStateClass.MEASUREMENT
    _attr_attribution = "Data provided by DHL"
    _unrecorded_attributes = frozenset({"parcels"})
    ...
    def __init__(
        ...
        """Initialise the returning parcels sensor."""
        super().__init__(coordinator)
        self._user_info = user_info
        user_id: str = user_info.get("userId", "")
        self._attr_unique_id = f"{user_id}_returning_parcels"
        self._attr_device_info = _build_device_info(user_info)

    @property
    def native_value(self) -> int:
        """Return the number of active return parcels."""
        return len(self.coordinator.returning)

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the full list of active return parcels as an attribute."""
        return {"parcels": self.coordinator.returning}


class DhlDeliveredOutgoingParcelsSensor(CoordinatorEntity[DhlCoordinator], SensorEntity):
    """Sensor reporting return parcels that have arrived back at the shipper."""

    _attr_has_entity_name = True
    _attr_translation_key = "delivered_outgoing_parcels"
    _attr_state_class = SensorStateClass.MEASUREMENT
    _attr_attribution = "Data provided by DHL"
    _unrecorded_attributes = frozenset({"parcels"})
    ...
    def __init__(
        ...
        """Initialise the delivered outgoing (returns) sensor."""
        super().__init__(coordinator)
        self._user_info = user_info
        user_id: str = user_info.get("userId", "")
        self._attr_unique_id = f"{user_id}_delivered_outgoing_parcels"
        self._attr_device_info = _build_device_info(user_info)

    @property
    def native_value(self) -> int:
        """Return the number of delivered return parcels."""
        return len(self.coordinator.delivered_outgoing)

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the full list of delivered return parcels as an attribute."""
